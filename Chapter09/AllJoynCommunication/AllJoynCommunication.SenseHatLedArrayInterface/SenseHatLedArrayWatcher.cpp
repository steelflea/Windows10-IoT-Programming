...
SenseHatLedArrayWatcher::SenseHatLedArrayWatcher(AllJoynBusAttachment^ busAttachment) :
...
SenseHatLedArrayWatcher::~SenseHatLedArrayWatcher()
{
    UnregisterFromBus();
}

void SenseHatLedArrayWatcher::UnregisterFromBus()
{
    if (nullptr != m_aboutListener)
    {
        PCSTR interfaces[] = { "com.iot.SenseHatLedArray" };
...
void SenseHatLedArrayWatcher::OnAnnounce(
    _In_ PCSTR name,
    ...
    _In_ const alljoyn_msgarg aboutDataArg)
{
    ...
    if (alljoyn_aboutobjectdescription_hasinterface(objectDescription, "com.iot.SenseHatLedArray"))
    {
        AllJoynServiceInfo^ args = ref new AllJoynServiceInfo(
            AllJoynHelpers::MultibyteToPlatformString(name),
            AllJoynHelpers::GetObjectPath(objectDescription, "com.iot.SenseHatLedArray"),
            port);
        Added(this, args);
    }
...
void SenseHatLedArrayWatcher::BusAttachmentStateChanged(_In_ AllJoynBusAttachment^ sender, _In_ AllJoynBusAttachmentStateChangedEventArgs^ args)
{
    if (args->State == AllJoynBusAttachmentState::Connected)
    {
        alljoyn_aboutlistener_callback callbacks = 
        {
            AllJoynHelpers::AnnounceHandler<SenseHatLedArrayWatcher>
        };
...
void SenseHatLedArrayWatcher::Start()
{
    ...
    int32 result = AllJoynHelpers::CreateInterfaces(m_busAttachment, c_SenseHatLedArrayIntrospectionXml);
    ...
    m_busAttachmentStateChangedToken = m_busAttachment->StateChanged += ref new TypedEventHandler<AllJoynBusAttachment^, AllJoynBusAttachmentStateChangedEventArgs^>(this, &SenseHatLedArrayWatcher::BusAttachmentStateChanged);
    m_busAttachment->Connect();
}

void SenseHatLedArrayWatcher::Stop()
{
    StopInternal(AllJoynStatus::Ok);
}

void SenseHatLedArrayWatcher::StopInternal(int32 status)
{
    UnregisterFromBus();
